...
"""
GALLOWS = [{"f":f"{Fore.GREEN+Back.BLACK+Style.NORMAL}   ","p":f"{Fore.GREEN+Style.NORMAL}_________"},   # Gallows elements for printing.
           {"f":f"{Fore.GREEN+Style.NORMAL}  |         ","p":f"{Fore.YELLOW+Style.NORMAL}|"},  # "f" --> gallows frame, always
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f"{Fore.WHITE+Style.DIM} O"},  #         displayed
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f"{Fore.MAGENTA+Style.DIM} |"},  # "p" --> hangman person, to be
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f"{Fore.MAGENTA+Style.DIM}---"}, #         displayed as wrong guesses 
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f"{Fore.BLUE+Style.DIM} | "}, #         are made
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f"{Fore.BLUE+Style.DIM}/ \\"},
           {"f":f"{Fore.GREEN+Style.NORMAL}  |        ","p":f""},
           {"f":f"{Fore.GREEN+Style.NORMAL}__|__      ","p":f""}]
...
def displayHeader(score):
    ...
    print(f"{Fore.YELLOW+Style.BRIGHT}*** {Fore.BLUE}SuniTAFE - Hangman {Fore.YELLOW+Style.BRIGHT}*** {Fore.RED+Style.NORMAL}Success rate: {Style.BRIGHT}{successRate}%")
...
def displayGuesses(guesses):
    ...
    Parameters:
        guesses (list of characters): All letters guesses, both correct and 
                                      incorrect.
    """
    print(f"{Fore.RED+Style.NORMAL}Guesses: {Style.BRIGHT}" + " ".join(guesses))
...
def displayWord(word, guesses):
    ...
    """
    foundCount = 0
    print(f"{Fore.GREEN+Style.NORMAL}Word: {Style.BRIGHT}", end="")
...
def main():
    ...
    * The word currently being guessed showing correctly guessed letters in 
    ...
    # initialise the coloured text system
    init()
    ...
    # Checknumber of command line arguments
    # More than one argument is an error
    if argCount > 2:
        print("Usage: hangman.py [wordFile]")
    else:
        
        # No argument, use default word file
        if argCount == 1:        
            wordFile = WORDFILE
            success = True
            
        # One argument, use argument as word file name
        elif argCount == 2:
            wordFile = sys.argv[1]
            
            # Check that word file exists
            if os.path.exists(wordFile):
                success = True
            else:
                print(f"Error: Word file {wordFile} does not exist.")

        # Successfully selected word file
        if success:
            ...
            while True:
                ...
                while True:
                    ...
                    while True:
                        ...
                        guess = input(f"{Fore.BLUE+Style.NORMAL}Guess: {Style.BRIGHT}").upper()
                    ...
                    # Append the current guess to the list of all guesses.
                ...
                print(f"{Fore.YELLOW+Style.BRIGHT}Well done, you won!" if youWon else (f"{Fore.YELLOW+Style.BRIGHT}Sorry, you lost!  The word was " + word))
